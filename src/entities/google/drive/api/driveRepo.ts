...
export const driveRepo = {
  listFolder: async (token: string, folderId: string): Promise<DriveFile[]> => {
  ...
  getMeta: async (token: string, id: string): Promise<DriveFile> => {
    const { data } = await googleClient
  ...
  getContent: async (token: string, id: string): Promise<string> => {
    const { data } = await googleClient
  ...
  updateFile: async (
    ...
    const current = await driveRepo.getMeta(token, id);
  ...
  renameFile: async (token: string, id: string, name: string): Promise<DriveFile> => {
    const { data } = await googleClient
  ...
  trashFile: async (token: string, id: string): Promise<void> => {
    await googleClient
