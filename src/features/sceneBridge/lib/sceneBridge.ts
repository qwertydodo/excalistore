...
export const readScene = async (deps: SceneBridgeDeps): Promise<ExcalidrawFile> => {
...
export const writeScene = async (file: ExcalidrawFile, deps: SceneBridgeDeps): Promise<void> => {
...
export const clearScene = async (deps: SceneBridgeDeps): Promise<void> => {
...
export const readTheme = (deps: SceneBridgeDeps): ThemeMode => {
  const appState = readJson<{ theme?: string }>(deps.storage, STATE_KEY, {});
  return appState.theme === THEME_MODE.DARK ? THEME_MODE.DARK : THEME_MODE.LIGHT;
};

// Hash of the current scene for autosave change-detection (Plan 4).
export const currentSceneHash = async (deps: SceneBridgeDeps): Promise<string> => {
